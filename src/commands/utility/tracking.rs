...
pub async fn track_user(
    ...
    if let Some(user) = user {
        ...
        if let Some(member) = guild_id.member(ctx.http(), tracked_user_id).await.ok() {
            ctx.say(format!(
                "{} is already a member of this guild and cannot be tracked.",
                member.display_name()
            ))
            .await?;
        } else if is_user_already_tracking(&ctx, guild_id, tracked_user_id, author_id, ctx.data())
            .await
        {
...
async fn is_user_already_tracking(
    _ctx: &Context<'_>,
    guild_id: GuildId,
    tracked_user_id: UserId,
    author_id: UserId,
...
#[poise::command(
    rename = "tracked-users",
    slash_command,
    prefix_command,
    aliases("tracked_users", "listtracked", "list-tracked"),
    guild_only,
    category = "Utility"
)]
pub async fn tracked_users(ctx: Context<'_>) -> Result<(), Error> {
    let guild_id = ctx.guild_id().unwrap();
    let author_id = ctx.author().id;
    ...
    if !tracked_users.is_empty() {
        ...
        ctx.say("You are not tracking any users in this guild.")
            .await?;
    }
            ...
            for row in rows {
                if let Some(user_id) = row.user_id {
                    if let Some(user_name) =
                        fetch_user_name(&ctx, UserId(NonZeroU64::new(user_id as u64).unwrap()))
                            .await
                    {
                        tracked_users
                            .push((user_name, UserId(NonZeroU64::new(user_id as u64).unwrap())));
                    }
                }
            }
