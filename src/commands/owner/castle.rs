use crate::{Context, Error, config::{InitStatus, Castle, StatsStatus, SelfRegex, MediaStashingStatus}};

use poise::serenity_prelude::{
    ButtonStyle, ComponentInteractionCollector, CreateActionRow, CreateButton, CreateInteractionResponse, CreateInteractionResponseMessage, CreateChannel, ChannelType
};
use regex::Regex;

/// Init
#[poise::command(rename = "init", prefix_command, hide_in_help, owners_only, guild_only, subcommands("force"))]
...
async fn setup(ctx: Context<'_>) -> Result<(), Error> {
    ...
    // Check if this guild is okay, run through settings, modify config.
            ...
            .await
        {
            ...
            if press.data.custom_id == okay_id {
                press.create_response(ctx.serenity_context(), CreateInteractionResponse::Acknowledge).await?;
                response = true;
                setup_settings(ctx).await?;

            } else if press.data.custom_id == not_okay_id {
                press.create_response(ctx.serenity_context(), CreateInteractionResponse::UpdateMessage(
                    CreateInteractionResponseMessage::new().content("Aborted.").components(vec![])
                )).await?;
                response = true;
            } else {
                continue;
    ...
    if !response {
        msg.edit(ctx, poise::CreateReply::default().content("Init process timeout.").components(vec![])).await?;
    }
    Ok(())
}
...
async fn setup_settings(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let category_id = guild_id.create_channel(ctx, CreateChannel::new("jamespy").kind(ChannelType::Category)).await?.id;


    // Enable stats?
    let stats_id = guild_id.create_channel(ctx, CreateChannel::new("stats").kind(ChannelType::Text).category(category_id)).await?.id;
    let stats_message = stats_id.say(ctx, "initializing...").await?.id;


    let regex_id = guild_id.create_channel(ctx, CreateChannel::new("regex-mentions").kind(ChannelType::Text).category(category_id)).await?.id;
    // Enable regex allocation channel?
    // - ask if want ping.
    // accept regexes in model, encode.

    let media_stashing_id = guild_id.create_channel(ctx, CreateChannel::new("deleted-media").kind(ChannelType::Text).category(category_id)).await?.id;
